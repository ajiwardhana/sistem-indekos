            ...
            <form action="{{ route('pembayaran.update', $pembayaran->id) }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-control @error('penyewaan_id') is-invalid @enderror" 
                                ...
                                <option value="{{ $penyewaan->id }}" 
                                    @selected(old('penyewaan_id', $pembayaran->penyewaan_id) == $penyewaan->id)>
                                    {{ $penyewaan->nama }} - Kamar {{ $penyewaan->kamar->nomor_kamar }}
                                </option>
                                @endforeach
                            </select>
                            @error('penyewaan_id')
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="month" class="form-control @error('bulan') is-invalid @enderror" 
                                   id="bulan" name="bulan" 
                                   value="{{ old('bulan', \Carbon\Carbon::parse($pembayaran->bulan)->format('Y-m')) }}" required>
                            @error('bulan')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="col-md-4">
                        <div class="mb-3">
                            ...
                            <input type="date" class="form-control @error('tanggal') is-invalid @enderror" 
                                   ...
                                   value="{{ old('tanggal', $pembayaran->tanggal->format('Y-m-d')) }}" required>
                            @error('tanggal')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="col-md-4">
                        <div class="mb-3">
                            ...
                            <select class="form-control @error('metode_pembayaran') is-invalid @enderror" 
                                ...
                                <option value="transfer" @selected(old('metode_pembayaran', $pembayaran->metode_pembayaran) == 'transfer')>Transfer</option>
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control @error('keterangan') is-invalid @enderror" 
                              id="keterangan" name="keterangan" rows="2">{{ old('keterangan', $pembayaran->keterangan) }}</textarea>
                    @error('keterangan')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
